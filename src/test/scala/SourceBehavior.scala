...
import concurrent.Async.Listener
...
class SourceBehavior extends munit.FunSuite {
  ...
  test("sources do not wait on zombie sources") { // currently fails, this would be better if it worked
    Async.blocking:
      val timeBefore = System.currentTimeMillis()
      val f = Future {
        Future { Thread.sleep(70) }
        10
      }.value
      val timeAfter = System.currentTimeMillis()
      assert(timeAfter - timeBefore <= 10)
  }

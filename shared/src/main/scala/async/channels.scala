...
object Channel:
  ...
  private[async] abstract class Impl[T] extends Channel[T]:
    ...
    override val readSource: Source[ReadResult]^{this} = new Source {
    ...
    private[async] class CellBuf():
      ...
      private val pending: mutable.Queue[(Cell^) @uncheckedCaptures] = mutable.Queue[(Cell^) @uncheckedCaptures]()
      ...
      def dropSender(src: CanSend, s: Sender^): this.type =
        if sender > 0 then if pending.removeFirst(_ == Send(src, s)).isDefined then sender -= 1
        this
      ...
      def cancel() =
        pending.foreach {
          case Send(src, s)  => s.completeNow(Left(Closed), src)
  ...
  private class Impl[T] extends ChannelMultiplexer[T]:
    ...
    def run()(using Async) = {
      ...
      while (!shouldTerminate) {
        ...
        val selectCases = infoCase +: publishersCopy.map: pub =>
          pub.readSource ~~> {
            case Right(v) =>
              val subscribersCopy = synchronized(subscribers.toList)
              var c = 0
